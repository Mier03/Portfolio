...
const ProjectModal = ({ isOpen, onClose, project }) => {
  ...
  const roles = project.role ? project.role.split(', ').map(role => role.trim()) : [];
  ...
  const projects = [
    {
      ...
      role: "👩‍💼Project Manager, 👩‍💻Full Stack Developer, 🛠️Bug Fixing, 🛠️Debugging",
      files: [
    ...
    },
    {
      ...
      role: "👩‍💻Full Stack Developer, 🛠️Bug Fixing , 🛠️Debugging",
      files: [
                      ...
                      <p className="text-gray-400 text-sm">
                        {displayedRoles.map((role, i) => (
                          <React.Fragment key={i}>
                            {role}
                            {i < displayedRoles.length - 1 && <br />}
                          </React.Fragment>
                        ))}
                        {showMore && <br />}
                        {showMore && <span className="text-blue-300">...</span>}
                      </p>
                    </div>
